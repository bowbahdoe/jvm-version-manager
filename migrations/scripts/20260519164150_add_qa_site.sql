...
CREATE FUNCTION identity.set_current_timestamp_updated_at()
    RETURNS TRIGGER AS $$
DECLARE
    _new record;
BEGIN
...
CREATE TRIGGER set_identity_organization_updated_at
    BEFORE UPDATE ON identity.organization
    FOR EACH ROW
EXECUTE PROCEDURE identity.set_current_timestamp_updated_at();
...
CREATE TRIGGER set_identity_organization_user_updated_at
    BEFORE UPDATE ON identity.organization_user
    FOR EACH ROW
EXECUTE PROCEDURE identity.set_current_timestamp_updated_at();

CREATE SCHEMA qa;

CREATE FUNCTION qa.set_current_timestamp_updated_at()
    RETURNS TRIGGER AS $$
DECLARE
    _new record;
BEGIN
...
CREATE TRIGGER set_qa_question_updated_at
    BEFORE UPDATE ON qa.question
    FOR EACH ROW
EXECUTE PROCEDURE qa.set_current_timestamp_updated_at();
...
DROP FUNCTION qa.set_current_timestamp_updated_at();
...
DROP FUNCTION identity.set_current_timestamp_updated_at();
