...
(defn enrich-jar
  ...
  (if (or (nil? out-path)
    ...
    (with-open [jar (JarFile. ^String in-path)]
      (let [module-info (if (string? module-info)
        ...
        (with-open [os      (io/output-stream out-path)
          ...
          (let [module-info-entry (JarEntry. "META-INF/versions/9/module-info.class")]
            (JarOutputStream/.putNextEntry jar-out module-info-entry)
...
(defn packages-in-jar
  ...
  (with-open [jar-file (if (instance? JarFile jar)
                         jar
                         (JarFile. ^String jar))]
    (let [entries (JarFile/.entries jar-file)]
      (loop [packages #{}]
        (if (.hasMoreElements entries)
          (let [entry (.nextElement entries)]
            (if (string/ends-with? (JarEntry/.getName entry) ".class")
              (if-let [package-name (with-open [is (JarFile/.getInputStream jar-file entry)]
                                      (-> (ClassFile/of)
